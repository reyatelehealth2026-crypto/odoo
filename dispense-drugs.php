...
<div class="container mx-auto px-4 py-6 max-w-5xl">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-800">จ่ายยา/สินค้า</h1>
            ...
            <div class="non-drug-section">
                <div class="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div class="col-span-2">
                        <label class="text-gray-600">คำเตือน/หมายเหตุ</label>
                        <input type="text" value="${drug.warning || ''}" onchange="updateDrug(${idx}, 'warning', this.value)"
                               class="w-full px-2 py-1.5 border rounded mt-1" placeholder="ถ้ามี">
                    </div>
                </div>
            </div>
            ` : `
...
function rejectCase() {
    ...
    .then(r => r.json())
    .then(data => {
        if (data.success) { alert('ส่งข้อความแจ้งลูกค้าแล้ว'); window.location.href = 'pharmacist-dashboard.php'; }
    });
}
</script>
