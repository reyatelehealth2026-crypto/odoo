/**
 * Inbox Intelligence Dashboard — v7 Dark Theme
 * Upgraded color palette: #0B0F19 bg, #151929 cards, #1e2435 borders.
 * Accent gradient: #3B82F6 → #8B5CF6. Fonts: Inter + Noto Sans Thai.
 */
'use strict';
...
function colorClass(c) {
  const map = {
    ...
    indigo: { text: 'c-indigo', badge: 'badge-blue',   hex: '#8B5CF6' },
  };
      ...
      if (trend.length) {
        ...
        mkChart('trendCanvas', {
          ...
          options: {
            ...
            scales: {
              x:  { grid: { color: '#1e2435' }, ticks: { color: '#64748b', maxRotation: 0 } },
              y1: { position: 'left',  grid: { color: '#1e2435' }, ticks: { color: '#64748b' }, title: { display: true, text: 'ออเดอร์', color: '#475569' } },
      ...
      if (hourly.length) {
        ...
        mkChart('hourlyCanvas', {
          ...
          data: {
            ...
            datasets: [
              { label: 'IN',  data: hLabels.map(h => inMap[h]  || 0), backgroundColor: 'rgba(59,130,246,0.7)', borderColor: '#3B82F6', borderWidth: 1, borderRadius: 3 },
          ...
          options: {
            ...
            scales: {
              x: { grid: { display: false }, ticks: { color: '#1e2435', maxTicksLimit: 13, callback: (v, i) => i % 4 === 0 ? `${i}h` : '' } },
              y: { grid: { color: '#1e2435' }, ticks: { color: '#64748b' } },
            },
          },
        });
      } else {
...
async function loadTrafficSection() {
  ...
  try {
    ...
    const rows = [
      { icon: '📨', label: 'ข้อความเข้า',   today: todayIn,    yday: ydayIn,    avg: avgIn,  col: '#8B5CF6' },
  ...
  try {
                ...
                return `<tr>
                  <td>
                    <span style="display:inline-flex;align-items:center;gap:6px;">
                      <span style="width:20px;height:20px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;background:rgba(59,130,246,0.15);flex-shrink:0;">${icon}</span>
    ...
    if (workEl) {
      if (!work.length) {
        ...
        const COLS = ['#3B82F6','#10b981','#f59e0b','#ef4444','#a78bfa','#14b8a6','#ec4899','#60a5fa','#f97316','#84cc16'];
        workEl.innerHTML = `
    ...
    if (tableEl && products.length) {
      tableEl.innerHTML = `
        <div style="overflow-y:auto;max-height:420px;">
          <table class="dt">
            <thead style="position:sticky;top:0;background:#151929;z-index:2;">
              <tr>
    ...
    mkChart('sentCanvas', {
      ...
      options: {
        ...
        scales: {
          ...
          y: { grid: { color: '#1e2435' }, ticks: { color: '#64748b', stepSize: 1 } },
        },
      },
    });
...
async function loadJourneySection() {
  ...
  try {
    ...
    if (stages.length > 0) {
      const FUNNEL = ['#3B82F6','#8b5cf6','#a855f7','#ec4899','#f43f5e'];
    ...
    if (fEl) {
      if (data.problematic_incoming.length === 0) {
        ...
        fEl.innerHTML = data.problematic_incoming.map(m => {
          ...
          return '<div style="background:#1e2435;border-radius:6px;padding:8px 10px;margin-bottom:6px;border-left:3px solid #fbbf24;">' +
    ...
    if (uEl) {
      if (data.inappropriate_outgoing.length === 0) {
        ...
        uEl.innerHTML = data.inappropriate_outgoing.map(m => {
          ...
          return '<div style="background:#1e2435;border-radius:6px;padding:8px 10px;margin-bottom:6px;border-left:3px solid #f87171;">' +
            '<div style="display:flex;justify-content:space-between;align-items:center;">' +
