/**
 * Inbox Intelligence Dashboard — v8.2 (Debug + Fixed)
 * JS Controller
 */
'use strict';
...
async function loadUnprofessional() {
  ...
  try {
    const res = await fetch(`${API}?action=unprofessional_messages&days=1`);
    const json = await res.json();
    const data = json.data || [];
...
async function loadAdminWorkload() {
  ...
  try {
    ...
    const maxMsgs = Math.max(...data.map(d => parseInt(d.message_count || 0)), 1);

    const html = data.map(d => {
      const pct = (parseInt(d.message_count || 0) / maxMsgs * 100).toFixed(0);
      ...
      return `
        <div style="margin-bottom:14px;">
          <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
            ...
            <span class="fs12 c-blue mono">${fmt(d.message_count)}</span>
          </div>
          <div class="wl-track">
